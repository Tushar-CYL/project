...
REDIRECT_URI = "https://adsync-uu8a.onrender.com"  # Update for production
    ...
    try:
        ...
        for batch in response:
            for row in batch.results:
                ...
                conversion_rate = (conversions_value / conversions * 100) if conversions > 0 else 0
...
if credentials:
    ...
    if 'customers' in st.session_state:
        ...
        customer_id = st.sidebar.selectbox("Select Google Ads Account", st.session_state.customers)
        ...
        if st.sidebar.button("🚀 Fetch Campaign Data", use_container_width=True):
                ...
                if not df.empty:
                    ...
                    with col1:
                        ...
                        st.markdown(f"""
                            <div class="metric-card">
                                <h3>Total Spend</h3>
                                <h1 style="color: #4CAF50;">${total_cost:,.2f}</h1>
                                <p>Total Advertising Cost</p>
                            </div>
                        """, unsafe_allow_html=True)
                    ...
                    with col2:
                        ...
                        st.markdown(f"""
                            <div class="metric-card">
                                <h3>Total Clicks</h3>
                                <h1 style="color: #2196F3;">{total_clicks:,}</h1>
                                <p>All Campaigns</p>
                            </div>
                        """, unsafe_allow_html=True)
                    ...
                    with col3:
                        ...
                        st.markdown(f"""
                            <div class="metric-card">
                                <h3>Impressions</h3>
                                <h1 style="color: #9C27B0;">{total_impressions:,}</h1>
                                <p>Total Views</p>
                            </div>
                        """, unsafe_allow_html=True)
                    ...
                    with col4:
                        ...
                        st.markdown(f"""
                            <div class="metric-card">
                                <h3>Avg. CTR</h3>
                                <h1 style="color: #FF9800;">{avg_ctr:.2f}%</h1>
                    ...
                    # 3D Scatter Plot
                    fig_3d = px.scatter_3d(df, x='Cost', y='Clicks', z='Impressions',
                                        color='Campaign Name', size='Conversions',
                                        title='3D Campaign Performance Analysis',
                                        template='plotly_dark')
                    fig_3d.update_layout(scene=dict(
                        xaxis_title='Cost ($)',
                        yaxis_title='Clicks',
                        zaxis_title='Impressions'
                    ))
                    st.plotly_chart(fig_3d, use_container_width=True)

                    # Treemap Visualization
                    fig_treemap = px.treemap(df, path=['Campaign Name'], values='Cost',
                                          color='CTR (%)', hover_data=['Conversions'],
                                          title='Cost Distribution by Campaign',
                                          color_continuous_scale='RdYlGn')
                    st.plotly_chart(fig_treemap, use_container_width=True)

                    # Time Series Animation
                    fig_animated = px.bar(df, x="Date", y="Cost", color="Campaign Name",
                                       animation_frame="Date", range_y=[0, df["Cost"].max()*1.1],
                                       title="Daily Spend Evolution")
                    st.plotly_chart(fig_animated, use_container_width=True)

                    # Radar Chart for Campaign Metrics
                    radar_df = df.groupby('Campaign Name').agg({
                        'CTR (%)': 'mean',
                        'Conv. Rate (%)': 'mean',
                        'Avg. CPC': 'mean'
                    }).reset_index()
                    
                    fig_radar = px.line_polar(radar_df, r='CTR (%)', theta='Campaign Name',
                                            line_close=True, title='Campaign Performance Radar',
                                            color='Conv. Rate (%)',
                                            color_discrete_sequence=px.colors.sequential.Plasma_r)
                    st.plotly_chart(fig_radar, use_container_width=True)

                    # Funnel Chart
                    funnel_df = df.groupby('Campaign Name').sum().reset_index()
                    fig_funnel = px.funnel(funnel_df, x='Impressions', y='Clicks', color='Campaign Name',
                                         title='Conversion Funnel Analysis')
                    st.plotly_chart(fig_funnel, use_container_width=True)

                    # Heatmap
                    heatmap_df = df.pivot_table(index='Campaign Name', columns='Date', 
                                              values='Conversions', aggfunc='sum')
                    fig_heatmap = px.imshow(heatmap_df, aspect='auto',
                                          labels=dict(x="Date", y="Campaign", color="Conversions"),
                                          title='Daily Conversion Heatmap')
                    ...
                    st.warning("No campaign data found for selected dates")
